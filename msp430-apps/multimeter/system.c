...
inline void dmaInit()
{
	DMACTL0 = DMA0TSEL1 | DMA0TSEL2; // ADC12IFG for DMA0
	DMACTL1 = 0;
	DMA0CTL = DMADT0|DMADT2 | DMADSTINCR1 | DMADSTINCR0 | DMAEN | DMAIE | DMASRCINCR0 | DMASRCINCR1 ;
	// increment source, dest. Repeated-block-mode. Enable interrupts, enable dma0
	DMA0SA = (uint16_t)&ADC12MEM0;
	DMA0DA = (uint16_t)&adcArray[0];
	DMA0SZ = 2; // two channels
}
...
inline void adcInit()
{
	...
	// ADC12OSC, Repeated Seq-of-channels, no div on CLK, SAMPCON = sampling timer, SHS = ADC12SC, STARTADD = 0
	ADC12CTL1 = SHP | CONSEQ0 | CONSEQ1;
